...
void print(valarray<complex<double> > &v){
	
	for(int i = 0; i < v.size(); i++){
		cout << (int)round(v[i].real()) << ' ';
	}	
	cout << endl;
}

valarray<int> calc(string &str){
	
	int dg = str.size()-1;
	int r_dg = dg*2;
	int r_sz = r_dg+1;
	
	r_sz = 1 << int(ceil(log2(double(r_sz))));
	
	valarray<complex<double>> A(r_sz), B(r_sz);
	
	for(int i = 0; i < str.size(); i++){
		A[i]=B[str.size()-i-1] = str[i]-'0';
	}
	...
	valarray<int> ret(str.size()-1);
	
	for(int i = 1; i < str.size(); i++){
		ret[i-1] = round(A[str.size()+i-1].real());
	}
