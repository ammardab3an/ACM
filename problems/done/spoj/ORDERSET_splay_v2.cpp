...
int32_t main(){
    ...
    int t; cin >> t; while(t--){
        ...
        if(c=='I'){
            st.get_by_value(value, st.root);
            if(nodes[st.root].val != value){
                st.insert(value, st.root);
            }
        }
        else if(c=='D'){
            st.del(value, st.root);
        }
        else if(c=='C'){
            cout << st.lb(value, st.root) << endl;
        }
        else if(c=='K'){
            
            int k = value-1;
            if(k >= nodes[st.root].sz){
                cout << "invalid" << endl;
            }
            else{
                st.get_by_idx(k, st.root);
                cout << nodes[st.root].val << endl;
            }
        }
    }	
}
