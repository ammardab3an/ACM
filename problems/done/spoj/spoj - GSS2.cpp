...
struct node{
	int cur, mx;
	int lazy_pos, lazy_neg;	
	node(){
		cur=mx=lazy_pos=lazy_neg = 0;
	}
};
...
bool push(int _nd, int l, int r){
	...
	if(!nd.lazy_pos && !nd.lazy_neg){
		return false;
	}	
	
	if(nd.lazy_neg > 0){
		nd.lazy_pos += nd.lazy_neg;
		nd.lazy_neg = 0;
	}
	
	nd.cur += nd.lazy_pos;
	nd.mx = max(nd.mx, nd.cur);
	nd.cur += nd.lazy_neg;
	...
	if(l != r){
		...
		for(auto ch : {&tree[_nd*2], &tree[_nd*2+1]}){
			...
			if(ch->lazy_neg==0){
				ch->lazy_pos += nd.lazy_pos;
				ch->lazy_neg += nd.lazy_neg;
			}
			else{
				ch->lazy_neg += nd.lazy_pos;
				if(ch->lazy_neg > 0){
					ch->lazy_pos += ch->lazy_neg;
					ch->lazy_neg = 0;
				}
				ch->lazy_neg += nd.lazy_neg;
			}
