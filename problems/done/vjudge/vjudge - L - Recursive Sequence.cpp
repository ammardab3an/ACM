// Problem: L - Recursive Sequence
// Contest: Virtual Judge - Math Week 6
// URL: https://vjudge.net/contest/495976#problem/L
...
matrix fast_power(matrix &mat, int p){
	...
	int n = mat.n;
	matrix product(n);
	for(int i = 0; i < n; i++) product.vec[i][i] = 1;
...
int32_t main(){
    ...
    int t; cin >> t; while(t--){
		...
		for(int i = 0; i < n; i++){
			cin >> conv.vec[n-1][n-i-1];
		}
		
		int m;
		cin >> m;
		
		conv = fast_power(conv, m-1);
