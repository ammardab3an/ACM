    ...
    int t; cin >> t; while(t--){
		...
		auto check = [&](int k){
			...
			for(int i = n-1; i >= 0; i--){
				...
				int l = i+1;
				int r = (upper_bound(pre.begin(), pre.end(), pre[i]+k)-pre.begin());
				
				int cans = vec[i] + dp.query(1, 0, n, l, r);
				dp.update(1, 0, n, i, cans);
			}
			
			for(int i = 0; i < n; i++){
				if(!i || pre[i-1] <= k){
					if(dp.query(1, 0, n, i, i) <= k){
						return true;
					}
				}
			}
