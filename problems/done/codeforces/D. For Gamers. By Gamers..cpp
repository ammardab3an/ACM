...
int32_t main(){
    ...
    vector<pair<int32_t, int32_t>> tmp;
    
    for(int i = 0; i < n; i++){
    	int a = vec[i].first;
    	int b = vec[i].second;
    	for(int j = 1; j*a <= c; j++){
    		// tmp.push_back({j*b, j*a});
    		tmp.push_back({i, j});
    	}
    }
    
    sort(tmp.begin(), tmp.end(), [&](const pii &a, const pii &b){
    	return a.second*vec[a.first].second < b.second*vec[b.first].second;
    });
    
    vi suf(tmp.size()+1);
    suf.back() = INFLL;
    for(int i = (int(tmp.size()))-1; i >= 0; i--){
    	suf[i] = min(suf[i+1], (int)(tmp[i].second*vec[tmp[i].first].first));
    }
    ...
    int m; cin >> m; while(m--){
    	...
    	while(l <= r){
    		...
    		int p_mid = tmp[mid].second*vec[tmp[mid].first].second;
