...
int32_t main(){
    ...
    for(int i = 0; i < n; i++) if(arr[i]) dp[arr[i]] += 1;
    
    for(int i = 0; i < LOG; i++) for(int msk = (1 << LOG)-1; msk >= 1; msk--){
    ...
    for(int i = 1; i < (1 << LOG); i++){
        ans = add(ans, __builtin_popcount(i)%2 ? pp[dp[i]] : -pp[dp[i]]);
    }
    
    cout << add(pp[n], -ans) << endl;
}
