...
int32_t main(){
    ...
    int t; cin >> t; while(t--){
		...
		vi vec_h(n), vec_p(n);
		for(auto &i : vec_h) cin >> i;
		for(auto &i : vec_p) cin >> i;
		
		multiset<pii> st0, st1;
		for(int i = 0; i < n; i++){
			st0.insert({vec_h[i], vec_p[i]});
			st1.insert({vec_p[i], vec_h[i]});
		}
		...
		while(!st0.empty() && k > 0){
			...
			water_level += k;
			while(!st0.empty() && (st0.begin()->first)-water_level <= 0){
				auto p = *st0.begin();
				st0.erase(st0.find(p));
				swap(p.first, p.second);
				st1.erase(st1.find(p));
			}	
