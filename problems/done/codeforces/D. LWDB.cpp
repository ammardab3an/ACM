...
int dist(int u, int v){
	int g = lca(u, v);
	return dist_par(u, g) + dist_par(v, g);
}
...
void dfs0(int u, int p){
	...
	for(auto [v, w] : adj[u]) if(v != p){
		...
		par_dist[v][0] = w;
		for(int i = 1; i < log_n; i++){
			par_dist[v][i] = par_dist[v][i-1] + par_dist[par[v][i-1]][i-1];
		}
