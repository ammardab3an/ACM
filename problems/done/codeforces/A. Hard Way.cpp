...
int32_t main(){
    ...
    int t; cin >> t; while(t--){
		...
		for(int i = 0; i < 3; i++){
			...
			if(cor[i].second >= cor[(i+1)%3].second && cor[i].second >= cor[(i+2)%3].second){
				
				if(cor[i].second == cor[(i+1)%3].second){
					ans += dist(cor[i], cor[(i+1)%3]);
				}
				
				if(cor[i].second == cor[(i+2)%3].second){
					ans += dist(cor[i], cor[(i+2)%3]);
				}
			}
		}
