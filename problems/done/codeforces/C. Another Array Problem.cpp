...
int32_t main(){
    ...
    int t; cin >> t; while(t--){
		...
		if(n==2){
			int st_path = vec[0]+vec[1];
			int nd_path = abs(vec[0]-vec[1])*2;
			cout << max(st_path, nd_path) << endl;
		}
		else if(n==3){
			
			int mx = *max_element(vec.begin(), vec.end());
			
			if(mx==vec[0] || mx==vec[2]){
				cout << mx*n << endl;
			}
			else{
				int ans0 = vec[0]+vec[1]+vec[2];
				int ans1 = abs(vec[0]-vec[1])*3;
				int ans2 = max({vec[0], vec[2], abs(vec[0]-vec[1]), abs(vec[1]-vec[2])}) * 3;
