...
int go_0(int i){
    ...
    if(vis[i] == vid) return mem[i];
    vis[i] = vid;
    
    int stPath = pow_exp(pow_exp(2, k-i-1), n);
    int ndPath = mul(tmp_cho, go_0(i+1));
    
    return add(stPath, ndPath);
}
...
int go_1(int i){
    ...
    int stPath = mul(add(1, tmp_cho), go_1(i+1));
    int ndPath = 0;
