...
int32_t main(){
    ...
    int t; cin >> t; while(t--){
		...
		for(int i = 0; i < 32; i++){
			...
			for(auto p : vec) if((p.second>>i)&1){
				tmp.push_back(p);
			}
			
			int lst_r = -1;
			for(auto p : tmp) if(p.first.first > lst_r){
				lst_r = p.first.second;
				cnt[i]++;
			}
		}
		
		int ans = 0;		
		for(int i = 0; i < 32; i++) if(cnt[i]){
			int cans = mul(1ll << i, pow_exp(2, n-cnt[i]));
			cans = mul(cans, pow_exp(2, cnt[i]-1));
			ans = add(ans, cans);
		}
