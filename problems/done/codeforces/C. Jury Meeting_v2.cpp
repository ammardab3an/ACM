...
int32_t main(){
    ...
    int t; cin >> t; while(t--){
        ...
        int cnt = lower_bound(vec.begin(), vec.end(), vec[n-1]) - lower_bound(vec.begin(), vec.end(), vec[n-1]-1);
        
        int ans = fac[n];
        int n_ans = mul(fac[n], mul(fac[cnt], ifac[cnt+1]));
        ans = add(ans, mul(-1, n_ans));
