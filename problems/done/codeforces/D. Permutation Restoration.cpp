...
int32_t main(){
    ...
    int t; cin >> t; while(t--){
		...
		set<iii> tot;
		for(int i = 0; i < n; i++){
			tot.insert({tmp[i].first, {i, tmp[i].second}});
		}
		
		set<pii> st;
		for(int i = 1; i <= n; i++){
			
			while(!tot.empty() && ((tot.begin()->first) <= i)){
				iii tmp = *tot.begin();
				tot.erase(tot.begin());
				st.insert({tmp.second.second, tmp.second.first});
			}
