...
int frq[NMAX], cnt[NMAX], sfrq[BLOCK];
int ans[NMAX];
...
int squery(int i){
    int ret = 0;
    for(int j = i/BLOCK+1; j < BLOCK; j++) ret += sfrq[j];
    for(int j = i; (j <= n) && (i/BLOCK == j/BLOCK); j++) ret += frq[j];
    return ret;    
}

void add(int c){
    if(cnt[c]) sadd(cnt[c], -1);
    sadd(++cnt[c], +1);
}
