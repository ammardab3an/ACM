...
int32_t main(){
    ...
    for(int i = 0; i < n; i++){
        ...
        st.insert({ai-st_add, -(1-nd_add)});
        
        while(st.size() && ((st.begin()->first+st_add) < 0)){
            st.erase(st.begin());
        }
        
        auto it = st.lower_bound({0-st_add, -INF});
        
        if((it != st.end()) && ((it->first+st_add) == 0)){
            
            int cur_len = -(it->second)+nd_add;
