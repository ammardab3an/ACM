...
double go(int i, bool b){
    ...
    double st_path = go(i+1, b);
    double nd_path = go(i+1, !b) + (!b ? arr[i] : -arr[i]);
    
    return mem[i][b] = max(st_path, nd_path);
}

void calc(int i, int b){
